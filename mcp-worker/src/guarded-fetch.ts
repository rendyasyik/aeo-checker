/**
 * Remote MCP Worker guarded fetch — a THIN adapter over the shared core
 * (`src/guarded-fetch.ts`), IDENTICAL in policy to the REST Worker's
 * `worker/src/guarded-fetch.ts`. The SSRF guard, manual redirect
 * re-validation and depth cap all live in the shared core so the remote MCP,
 * the REST Worker and the Node MCP cannot drift. This file keeps only the two
 * Worker-specific bits:
 *
 ...
 *
 * The Workers runtime has no DNS API, so NO resolver is injected; the layered
 * synchronous SSRF checks + per-hop redirect re-validation apply (see
 * src/ssrf.ts for the honest DNS-rebinding limit). The remote MCP is a PUBLIC
 * surface, so `allowPrivateHosts` is ALWAYS false here — there is no opt-out.
 */
...
/** Sentinel prefix so callers can detect an SSRF block. */
...
export function makeGuardedFetch(
  ...
  return makeGuardedFetchCore({
    ...
    allowPrivateHosts: false, // Public surface; never opt out.
