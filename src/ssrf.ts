/**
 * SSRF guard (mitigation layer 5) — SHARED core-lib source of truth.
 *
 * Lifted from `worker/src/ssrf.ts` into the core lib (Fase 3) so the same guard
 * protects every fetch path: the Cloudflare Worker (web tool), the MCP server,
 * and the site-scan discovery/sampling module which fetches many same-origin
 * URLs. The worker re-exports this module to avoid behavioural drift.
 *
